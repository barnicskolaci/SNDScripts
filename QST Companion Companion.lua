--[=====[
[[SND Metadata]]
version: 2.3
triggers:
- onlogin
- onterritorychange
function_chat_filters:
  OnChatMessage:
    channels:
    - errormessage
    message_contains: Unable to teleport. Another teleport is already underway.
  OnChatMessage2:
    channels:
    - systemmessage
    message_contains: This ferry will become available upon reaching level 15.
[[End Metadata]]
--]=====]

import("System.Numerics")

local tp_error = false
local ferry_error = false
local interactioncount = 0
local targetName = nil
local debug = false
local stoplevel = 47

function TerritoryType()
    if Svc.ClientState.TerritoryType then
        return Svc.ClientState.TerritoryType
    end 
end

function QuestText()
    if not Addons.GetAddon("_ToDoList").Exists then
        return ""
    end
    if Addons.GetAddon("_ToDoList"):GetNode(1,20101,6).IsVisible then
        return Addons.GetAddon("_ToDoList"):GetNode(1,20101,6).Text
    elseif Addons.GetAddon("_ToDoList"):GetNode(1,20101,7).IsVisible then
        return Addons.GetAddon("_ToDoList"):GetNode(1,20101,7).Text
    else
        return ""
    end
end

function IsInSanctuary()
    if not Addons.GetAddon("_Exp").Exists then
        return false
    end
    return Addons.GetAddon("_Exp"):GetNode(1,3).IsVisible
end

function OnChatMessage()
    if tp_error == false then
        tp_error = true
        if debug then
            Engines.Native.Run("/echo [QST] Debug: Teleport error detected.")
        end
    end
end

function OnChatMessage2()
    if ferry_error == false then
        ferry_error = true
        if debug then
            Engines.Native.Run("/echo [QST] Debug: Ferry error detected.")
        end
    end
end

function CloseCheck(a, b, tol) --tol is squared
    tol = tol or 4
    return ((a.X-b.X)^2 + (a.Y-b.Y)^2 + (a.Z-b.Z)^2) < tol
end

function IsPlayerCloseTo(x, y, z, deltasquared) --delta is squared
    if Entity.Player == nil or Entity.Player.Position == nil then
        return false
    end
    local p = Entity.Player.Position
    deltasquared = deltasquared or 4
    local result = CloseCheck(p, {X=x, Y=y, Z=z}, deltasquared)
    if debug then
        yield("/echo [QST] Debug: Checking if player is close to (" .. x .. ", " .. y .. ", " .. z .. ") with delta^2 " .. deltasquared .. " - Result: " .. tostring(result))
    end
    return result
end


-- Initialize position history

local positionHistory = {}

function CheckPosStuck()
    if not Entity.Player or not Player.Available then
        return false
    end
    
    local currentPos = Entity.Player.Position
    
    -- Handle nil position and combat/casting states
    if currentPos == nil or not Player.Available or Entity.Player.IsInCombat or Entity.Player.IsCasting then --i really hope nothing will periodically trigger these while the toon is still stuck
        positionHistory = {}
        return false
    end
    
    -- Add current position to history
    table.insert(positionHistory, currentPos)
    
    -- Keep only last 3 positions
    if #positionHistory > 3 then
        table.remove(positionHistory, 1)
    end
    
    -- Debug: yield position history
    if debug then
        yield("/echo Position History: " .. tostring(#positionHistory) .. " entries")
        for i, pos in ipairs(positionHistory) do
            yield("/echo [" .. i .. "] " .. tostring(pos))
        end
    end
    
    -- Check if we have 3 positions and they're all the same
    if #positionHistory == 3 then
        local pos1 = positionHistory[1]
        local pos2 = positionHistory[2]
        local pos3 = positionHistory[3]
        
        -- Compare Vector3 objects directly
        if CloseCheck(pos1, pos2, 2) and CloseCheck(pos2, pos3, 2) then
            yield("/echo Player is not moving anywhere.")
            return true
        end
    end
    
    return false
end        

function sleep(seconds)
    yield('/wait ' .. tostring(seconds))
end

function MoveAndInteract(target)
    if debug then
        yield("/echo [QST Comp Comp] Debug: Moving to and interacting with target: " .. target)
    end
    yield('/rsr off') -- this is needed cause qst companion turns it on and it prevents targetting
    yield('/vbm ar disable') --same
    yield('/target '.. target)
    local TargetAttempts = 0
    while not (Entity.Target and Entity.Target.Name == target) and TargetAttempts < 20 do
        sleep(0.100)
        TargetAttempts = TargetAttempts + 1
    end
    if Entity.Target and Entity.Target.Name == target then
        yield('/vnav movetarget')
        local mvAttempts = 0
        while Entity.Target and Entity.Target.DistanceTo > 3 and mvAttempts < 60 do
            sleep(0.101)
            mvAttempts = mvAttempts + 1
        end
        yield("/vnav stop")
        yield('/interact')
    end
end

local addonConfigs = {
    { addon = "TelepotTown", command = function() yield("/send ESCAPE") end }, --this might be fine, but needs testing; needs attention
    { addon = "Shop" }, --Dressed to Call: stuck on Buyback tab of shop
    { addon = "SelectYesno", condition = tp_error == false and function() return not IPC.Lifestream.IsBusy() end },
    { addon = "SelectString" },
    { addon = "JournalAccept" }, --qst stuck on war quest waiting to equip soul crystal
    { addon = "JournalResult", command = function() yield("/click JournalResult Decline") end },
    { addon = "JournalRewardItem" },
    { addon = "GrandCompanySupplyList" },
    { addon = "Description" }, --stuck on frontline info
    { addon = "Repair" },
    { addon = "GrandCompanyExchange" }, --stuck on GC exchange
    { addon = "PvpWelcome"}
}

function AddonHandler(addonConfigs)
    for _, config in ipairs(addonConfigs) do
        local Addon = config.addon
        local condition = config.condition

        -- Check custom condition if provided
        if condition and not condition() then
            goto skip_addon
        end

        if Entity.Player and Entity.Player.IsCasting then
            goto skip_addon
        end

        local attempts = 0
        local maxAttempts = config.maxAttempts or 3
        local waitBetween = config.waitBetween or 1.198

        while Addons.GetAddon(Addon).Ready and attempts < maxAttempts do
            if config.command then
                config.command()
                if debug then
                    yield("/echo [QST Comp Comp] Debug: Handling addon (custom): " .. Addon)
                end
            else
                if Addons.GetAddon(Addon).Ready then
                    -- Prefer callback when addon GUI is ready
                    yield('/callback "'.. Addon ..'" true -1')
                end
                if debug then
                    yield("/echo [QST Comp Comp] Debug: Handling addon: " .. Addon .. " (attempt " .. tostring(attempts+1) .. ")")
                end
            end

            attempts = attempts + 1
            sleep(waitBetween)
        end

        if debug and Addons.GetAddon(Addon).Ready then
            yield("/echo [QST Comp Comp] Warning: addon '" .. Addon .. "' still exists after " .. tostring(attempts) .. " attempts")
            -- Fallback: try to close with keys
            yield('/send ESCAPE')
            yield('/send DECIMAL')
        end
        ::skip_addon::
    end
end

function HardTarget()
    yield('/vbm cfg aiconfig forbidactions false')
    sleep(0.103)
    yield('/vbm ai on')
    if debug then
        yield("/echo [QST Comp Comp] Debug: HardTarget function called.")
    end
    if not Player.Entity.Target or not Svc.Targets.Target or Svc.Targets.Target:IsHostile() == false then
        if debug then
            yield("/echo [QST Comp Comp] Debug: Checking conditions for HardTarget.")
        end
      yield('/send TAB')
      sleep(1.004)
    end
    sleep(1.005)

    if (Svc.Condition[56] or Svc.Condition[34]) and Player.Entity.Target then
        if (Svc.Targets.Target and Svc.Targets.Target:IsHostile() == true) or (tostring(Player.Entity.Target.Type):match("^EventObj")) then
            yield('/vnav movetarget')
        end
    end
end


local hunt_objective_targets = {
    ["Slay coeurl pups and collect coeurl pup whiskers."] = "Coeurl Pup",
    ["Slay antelope stags for their horns."] = "Antelope Stag",
    ["Slay ziz."] = "Ziz",
    ["Slay ice sprites and obtain their cores."] = "Ice Sprite",
}

function MatchHuntObjective(text)
    for prefix, creature in pairs(hunt_objective_targets) do
        if text:sub(1, #prefix) == prefix then
            return creature
        end
    end
    return nil
end

--[[#########################################
###########  SCRIPT START  ##################
###########################################]]

yield("/dps wloadall") --this is for Dhog Potato System window placement

while Addons.GetAddon("_DTR").Exists do
    local hunt_target = MatchHuntObjective(QuestText())
        -- Overworld mob hunting
    if Player.Job.Level and Player.Job.Level >= stoplevel then
        while not Entity.Player or not Player.Available do
            yield("/send ESCAPE")
            sleep(0.506)
        end
        yield("/qst stop")
        if not IsInSanctuary() and not IPC.Lifestream.IsBusy() and not (Entity.Player and Entity.Player.IsCasting) then--not in sanctuary as per moon icon, and not already mid-teleport
            yield("/li limsa")
            while IPC.Lifestream.IsBusy() do
                sleep(0.9)
            end
        end
    elseif IPC.Questionable.IsQuestComplete("952") and not Svc.Condition[26] and not Svc.Condition[34] and not Svc.Condition[56] then --done pursuit of the past and is not in duty
        yield("/qst stop")
        if not IsInSanctuary() and not IPC.Lifestream.IsBusy() and not (Entity.Player and Entity.Player.IsCasting) then--not in sanctuary as per moon icon, and not already mid-teleport
            yield("/li limsa")
            while IPC.Lifestream.IsBusy() do
                sleep(0.901)
            end
        end
        sleep(1.9)
        yield("/autoduty run support 1042 1 false") --stone vigil
    elseif TerritoryType() == 1042 and IPC.AutoDuty.IsStopped() then
        yield("/ad start")
    elseif not IPC.Questionable.IsRunning() and not Svc.Condition[34] and not Svc.Condition[56] then
        AddonHandler(addonConfigs)
        sleep(0.306)
        yield("/qst start")
    end
    local counter_qid_343 = 0
    repeat --cycle wait replacement with check for qid 343
        sleep(1.267)
        counter_qid_343 = counter_qid_343 + 1
        if QuestText() == "Speak with the flame sergeant." and Entity.Player and Player.IsCasting then --lord of the inferno avoid tp=ing away
            MoveAndInteract("Flame Sergeant")
        end
        if (Svc.Condition[34] or Svc.Condition[56]) then
            if Svc.Condition[26] then
                yield('/vbm cfg aiconfig ForbidMovement False')
            else
                yield('/vbm cfg aiconfig ForbidMovement True')
            end
        end
    until counter_qid_343 >= 4
    if hunt_target then
    elseif Entity.Player and not Entity.Player.IsInCombat and not Svc.Condition[26] and not Svc.Condition[34] and not Svc.Condition[56] then
        yield('/vbm ai off')
        yield('/vbm cfg aiconfig forbidactions true')
    end
    if CheckPosStuck() and Entity.Player and not Entity.Player.IsCasting then
        if tp_error == true then
            yield("/return")
            yield('/waitaddon "SelectYesno"')
            if Addons.GetAddon("SelectYesno").Ready then
                yield("/click SelectYesno Yes")
            end            
            sleep(1.009)
            if Entity.Player and Entity.Player.IsCasting then
                tp_error = false
            end
            if debug then
                yield("/echo [QST Comp Comp] Debug: Teleport error detected and handled.")
            end
        elseif TerritoryType() == 128 and Entity.Player and Entity.Player.Position.Y > 90 then --stuck in landing pad
            yield("/return")
            yield('/waitaddon "SelectYesno"')
            if Addons.GetAddon("SelectYesno").Ready then
                yield("/click SelectYesno Yes")
            end
            sleep(1.334)
            if Entity.Player and Entity.Player.IsCasting then
                ferry_error = false
            else
              yield("/li limsa")
              ferry_error = false
            end
            if debug then
                yield("/echo [QST Comp Comp] Debug: Ferry error detected and handled.")
            end
        end

        --quest-related stuck checks
        local qid = IPC.Questionable.GetCurrentQuestId()
        if qid == "466" or qid == "621" or qid == "618" then -- getting stuck under darm dome and enemies not spawning
            yield('/send R')
            sleep(5.01)
            yield('/send R')
            if debug then
                yield("/echo [QST Comp Comp] Debug: Quest 466 detected, sending /send R twice.")
            end
        elseif qid == "889" then --! needs more specific instructions
            MoveAndInteract("Stolen Wares")
        elseif (TerritoryType() == 129 or TerritoryType() == 128) and qid == "546" and (QuestText() == "Speak with Baderon." or QuestText() == "Speak with the flame honor guard.") then
            yield("/qst stop")
            yield("/li The Aftcastle")
            while IPC.Lifestream.IsBusy() or not Entity.Player or not Player.Available do
                sleep(0.444)
            end
            sleep(2.424)
            if QuestText() == "Speak with the flame honor guard." then
                IPC.vnavmesh.PathfindAndMoveTo(Vector3(5.5, 39.9, 14.3), false)
                repeat
                    sleep(1.016)
                until not IPC.vnavmesh.IsRunning()
                MoveAndInteract("Skaenrael")
                yield('/waitaddon "SelectIconString"')
                if Addons.GetAddon("SelectIconString").Ready then
                    yield("/callback SelectIconString true 0")
                end    
                yield('/waitaddon "SelectYesno"')
                if Addons.GetAddon("SelectYesno").Ready then
                    yield("/click SelectYesno Yes")
                end
                sleep(5)
                MoveAndInteract("L'nophlo")
                yield('/waitaddon "SelectYesno"')
                if Addons.GetAddon("SelectYesno").Ready then
                    yield("/click SelectYesno Yes")
                end
            else
                yield('/vnav moveto 17.85372 40.21601 -4.443381')
                repeat
                    sleep(1.426)
                until not IPC.vnavmesh.IsRunning() 
                MoveAndInteract("Baderon")
            end
        elseif TerritoryType() == 156 and qid == "1005" and IsPlayerCloseTo(-65.19908, 2.9204268, -634.66235) then
            yield("/qst stop")
            MoveAndInteract("Wedge")
        elseif TerritoryType() == 156 and qid == "1005" and IPC.Questionable.GetCurrentStepData().InteractionType == "CompleteQuest" then --! this is no longer needed for me, needs testing
            yield("/qst stop")
            yield("/li dhona")
            sleep(10.018)
            MoveAndInteract("Slafborn")
            yield('/waitaddon "SelectYesno"')
            if Addons.GetAddon("SelectYesno").Ready then
                yield("/click SelectYesno Yes")
            end
        elseif qid == "343" and TerritoryType() == 146 and IsPlayerCloseTo(-63.111206, 5.673375, -554.7956) then --cooked quest path in lord of the inferno, handle if already tp-d away
            yield("/qst stop")
            yield("/li drybone")
            local counter = 0
            while TerritoryType() == 146 and counter < 10 do
                sleep(1.398)
                counter = counter + 1
            end
            sleep(3.399)
            MoveAndInteract("Adala")
            yield('/waitaddon "SelectYesno"')
            if Addons.GetAddon("SelectYesno").Ready then
                yield("/click SelectYesno Yes")
            end
            sleep(1.409)
        end
        while TerritoryType() == 134 and (qid == "462" or qid == "403") do --On to Summerford/relightting the torch(?)
            yield("/vnav moveto -52 33.3 164")
            sleep(5.011)
        end
        if (TerritoryType() == 198 or TerritoryType() == 128 or TerritoryType() == 129) and qid == "545" then --The Mizzenmast Repast
            if TerritoryType() == 198 then
                MoveAndInteract("Merlwyb")
            else
                yield('/target Mytesyn') --this could be reworked into moveandinteract()
                sleep(1.012)
                if Entity.Target and Entity.Target.Name == "Mytesyn" then
                    yield('/vnav movetarget')
                    while Entity.Target and Entity.Target.DistanceTo > 5 do
                        sleep(0.513)
                    end
                    yield("/vnav stop")
                    yield('/interact')
                    if debug then
                        yield("/echo [QST Comp Comp] Debug: Interacting with Mytesyn.")
                    end
                    while TerritoryType() == 128 do
                        yield('/send NUMPAD0')
                        sleep(1.014)
                    end
                    sleep(1.015)
                else
                    if qid == "545" and TerritoryType() ~= 177 then
                        yield("/li inn")
                        while IPC.Lifestream.IsBusy() do
                            sleep(1.016)
                        end
                    end
                end
            end
        end

        --Territory-specific stuck checks
        if TerritoryType() == 138 then --vbm moving to sahagin territory in western la noscea
            yield("/vbm ai off")            
            yield("/vbm ar clear")
        elseif TerritoryType() == 128 and Entity.Target and Entity.Target.Name == "L'nophlo" then --limsa landing pad
            yield("/qst stop")
            if debug then
                yield("/echo [QST Comp Comp] Debug: Interacting with Blanmhas.")
            end
            MoveAndInteract("Blanmhas")
            yield('/waitaddon "SelectIconString"')
            if Addons.GetAddon("SelectIconString").Ready then
                yield("/callback SelectIconString true 0")
            end    
            yield('/waitaddon "SelectYesno"')
            if Addons.GetAddon("SelectYesno").Ready then
                yield("/click SelectYesno Yes")
            end
        elseif (TerritoryType() == 130 and IsPlayerCloseTo(-20.7, 10, -41)) or (qid == "546" and QuestText() == "Speak with the flame honor guard.") then --uldah landing pad
            if IsPlayerCloseTo(-20.7, 10, -41) then
                yield("/qst stop")
                MoveAndInteract("Willahelm")
                if debug then
                    yield("/echo [QST Comp Comp] Debug: Interacting with Willahelm.")
                end
                yield('/waitaddon "SelectIconString"')
                if Addons.GetAddon("SelectIconString").Ready then
                    yield("/callback SelectIconString true 0")
                end    
                yield('/waitaddon "SelectYesno"')
                if Addons.GetAddon("SelectYesno").Ready then
                    yield("/click SelectYesno Yes")
                end
                sleep(10.399)
                yield("/qst stop")
            end
            MoveAndInteract("Flame Honor Guard")
        elseif TerritoryType() == 132 and IsPlayerCloseTo(20.5, -19, 111.3) then --gridania landing pad
            yield("/qst stop")
            MoveAndInteract("Lionnellais")
            if debug then
                yield("/echo [QST Comp Comp] Debug: Interacting with Lionnellais.")
            end
            yield('/waitaddon "SelectYesno"')
            if Addons.GetAddon("SelectYesno").Ready then
                yield("/click SelectYesno Yes")
            end
        --[[elseif IsPlayerCloseTo(-7.1, 40, -16) then --! idk what this is
            yield("/qst stop")
            IPC.vnavmesh.PathfindAndMoveTo(Vector3(13.7, 40, 70), false)
            sleep(10.016)]]
        elseif TerritoryType() == 145 and IsPlayerCloseTo(314.7, 23.3, -256.23) and Entity.Target and Entity.Target.Name == "Destination" then --!some quest i should specify
            yield("/qst stop")
            yield("/vbm ai off")
            yield("/vbm ar clear")
            local counter_destination_interact = 0
            while (not Entity.Player or not Entity.Player.IsCasting) and not IPC.Questionable.IsRunning() and counter_destination_interact < 10 do
                yield('/send NUMPAD0')
                sleep(1.017)
                counter_destination_interact = counter_destination_interact + 1
            end
        end
        
        --Addon handling for various stuck scenarios
        --this is before qst reload but after the wait to give time for qst to not get stuck again

        AddonHandler(addonConfigs)

        --testing to see if obsolete
        --[[if Entity.Target and tostring(Entity.Target.Type):match("^Aetheryte") and not IPC.Lifestream.IsBusy() then
            if debug then
                yield("/echo [QST Comp Comp] Debug: Stuck on aetheryte, attempting to let QST Companion handle it.")
            end
            yield("/qst reload")
            sleep(1.019)
            yield("/qst start")
            sleep(10.02)
            if Entity.Target and tostring(Entity.Target.Type):match("^Aetheryte") then
                if debug then
                    yield("/echo [QST Comp Comp] Debug: Interacting with aetheryte.")
                end
                yield("/interact")
            end]]
        --this shit never worked and i just want it to do stuff, so replacing the big brain idea to never do general interact if you have a target with just general interact. fingers crossed
        --[[elseif Entity.Target and targetName and Entity.Target.Name == targetName then --!this just never seems to trigger, idk why, plus idk if it's even needed
            interactioncount = interactioncount + 1
            if debug then
                yield("/echo [QST Comp Comp] Debug: trying to interact with " .. targetName .. " " .. tostring(interactioncount + 1) .. " times.")
            end
            yield("/interact")
            if interactioncount > 5 then
                if debug then
                    yield('/e [QST Comp Comp] Debug: interaction count exceeded 5, letting it go.')
                end
                interactioncount = 0
                sleep(70.021)
                targetName = nil
            end
        elseif Entity.Target then
            targetName = Entity.Target.Name
            if targetName == "Yellow Serpent Flyer" then
                -- Blacklist
            else
                if debug then
                    yield("/echo [QST Comp Comp] Debug: Target name set to " .. targetName)
                end
                yield("/interact")
            end
        else]]
        if Svc.Condition[34] and Svc.Condition[56] and IPC.AutoDuty.IsStopped() then --idk what this does in solo duties, hope nothing bad
            yield("/ad start")
        elseif hunt_target then
            if debug then
                yield("/echo [QST Comp Comp] Debug: Hunt objective detected, targeting " .. hunt_target)
            end
            yield("/vbm ai on")
            yield("/rsr manual")
            yield('/target "' .. hunt_target .. '"')
            if Entity.Target and Entity.Target.Name == hunt_target then
                yield("/vnav movetarget")
            end        
        elseif not Svc.Condition[26] and not Svc.Condition[34] and not Svc.Condition[56] and not IPC.Lifestream.IsBusy() and not Entity.Player.IsCasting and Player.Available then --pretty much hail merry attempt
            yield("/vbm ai off")
            yield("/vbm ar clear")
            yield('/vbm cfg aiconfig ForbidMovement True')
            yield("/qst stop")
            yield("/send NUMPAD0")
            yield("/send NUMPAD0")
            yield("/send NUMPAD0")
            sleep(2.573)
            yield("/qst start")
            sleep(9.575)
            if CheckPosStuck() then
                yield("/echo [QST Comp Comp] Debug: Standing still out of combat, reloading quest.")
                yield("/qst reload")
            end
        end
        AddonHandler(addonConfigs)
        yield('/vbm cfg aiconfig ForbidMovement False')
        positionHistory = {}
    end

    -- CheckPosStuck() end


    --General Battle NPC combat failing to start --idk why i took this out, looks useful if health never goes below QSTC trigger value
    --[[maybe it's to stop it from getting stuck fighting when unrelated mobs aggro, idk
    if Entity.Player and Entity.Player.IsInCombat and not Entity.Target then
        HardTarget()
    end
    if Entity.Target and tostring(Entity.Target.Type):match("^BattleNpc") then
        local target = Entity.Target.Name
        sleep(3.023)
        if Entity.Target and target == Entity.Target.Name and Entity.Player and not Entity.Player.IsInCombat then
            yield("/rsr manual")
            if debug then
                yield("/echo [QST Comp Comp] Debug: Battle NPC combat failed to start, sending /rsr manual.")
            end
            sleep(1.024)
        end
    end

    AddonHandler({
        {
    addon = "TelepotTown",--this might be fine, but needs testing
    condition = function() return not IPC.Lifestream.IsBusy() end,
    command = function()
        yield('/send ESCAPE')
        yield('/send DECIMAL')
        sleep(3.025)
    end
        }
    })]]

    --Solo duties and what have you
    local zones = { [249]=true, [280]=true, [274]=true, [307]=true, [330]=true, [278]=true, [301]=true, [263]=true, [305]=true } --castrum centri 305?
    if zones[Svc.ClientState.TerritoryType] then
        while (Svc.Condition[56] or Svc.Condition[34]) and not IPC.Lifestream.IsBusy() do
            HardTarget()
            sleep(2.025)
            --yield("/vbm ai off")
            yield('/rsr auto lowhp')
            yield('/target "drowned outranger"') --feint and strike
            yield('/target "draconian rosary"') --in the eyes of gods and men
            if TerritoryType() == 307 then
                yield('/vnav moveto 692.1429 66.79631 -299.17896') --feint and strike
                yield('/target "large boulder"') --the lominsan wayend
            elseif Entity.Target and Entity.Target.Name == "Wyvern" then --may need stg else
                yield('/send TAB')
                sleep(1.027)
            end
        end
        yield('/vbm cfg aiconfig forbidactions true')
    elseif (Svc.Condition[56] or Svc.Condition[34]) then
        HardTarget()
        sleep(1.661)
        yield("/vbm ai on")
        yield('/rsr auto lowhp')
    end

    --[[if IPC.Questionable.IsQuestAccepted("693") and not IPC.Questionable.IsQuestComplete("693") and Entity.Target and Entity.Target.Name  == "Swygskyf" then --!update coords, leve handler
        if debug then
            yield("/echo [QST Comp Comp] Debug: Attempting leve.")
        end
        yield('/waitaddon "GuildLeve"')
        if Addons.GetAddon("Journal").Ready then
            yield("/click JournalDetail AcceptMap")
        end
        IPC.vnavmesh.PathfindAndMoveTo(Vector3(613, 8.4, 477), false)
        while IPC.vnavmesh.IsRunning() do
            sleep(1)
        end        
        if not Addons.GetAddon("Journal").Exists then
            yield("/send J")
        end
        yield('/waitaddon "Journal"')
        if Addons.GetAddon("JournalDetail").Ready then
            yield("/click JournalDetail Initiate")
        end
        yield('/waitaddon "SelectYesno"')
        if Addons.GetAddon("SelectYesno").Ready then
            yield("/click SelectYesno Yes")
        end
        sleep(1)
        yield("/target crab")
        yield("/rsr manual")
        sleep(1)
        while Entity.Player and Entity.Player.IsInCombat do
            sleep(1)
        end
        yield('/waitaddon "SelectYesno"')
        if Addons.GetAddon("SelectYesno").Ready then
            yield("/click SelectYesno Yes")
        end
        MoveAndInteract("Swygskyf")
    end]]
end